...
* SortI, * SortII, * Seed, ** Route, ** BestRoute, ** CurrentRoute, RoutedCustomers, * Technology,
* Counter, * BestCounter, * CurrentCounter, * CounterStation, * BestCounterStation, * CurrentCounterStation,
CounterRemoval, SelectionNode1, SelectionNode2,* RemovedCustomer, * RemovalStation,* RemovalRoute, 
NoRoutes, BestNoRoutes, CurrentNoRoutes, NoRemovalC, * Score, * NoUse, NCR, NSR, NRR, NCI, NSI, 
NoOperators, IterC, IterS, EnergyLimit, TimeWindowCheck, Iter, IterO;

float* Xcoordinate, * Ycoordinate, * fCost, * RCost, * TCost, * CurrentTCost, * BestTCost, * MaxCapacity,
* eTime, * lTime, * ServiceTime, * Rate, ALNSStartTime, ALNSEndTime, * Demand,  MaxDistance,
* MaxEnergyCapacity, * CurrentMaxEnergyCapacity, * BestMaxEnergyCapacity,
* EnergyConsumption, * BestEnergyConsumption, * CurrentEnergyConsumption,
* AcquisitionCost, * CurrentAcquisitionCost, * BestAcquisitionCost,
* RouteLength, * BestRouteLength, * CurrentRouteLength, Cost, BestCost, CurrentCost, 
** Weight, ** Probability, * Pr, RouletteWheelPar, BigNumber, Epsilon, Ratio, // 1 = 0.96	2 = 0.62	3 = 0.82	4 = 0.58	5 = 0.76
*** vPtr, ** qPtr, ** yPtr, ** uPtr, *** TempV, ** TempQ, * TempU, ** TempY, ** t, ** d,
*** BestV, ** BestQ, ** BestU, ** BestY, *** CurrentV, ** CurrentQ, ** CurrentU, ** CurrentY;
...
void RemoveCustomer(int Variable1, int Variable2) {
	...
	if (MaxEnergyCapacity[Variable1] < EnergyLimit) {
		...
		//Emissions = Emissions + (Lambda * CurbWeight * Rho * Sigma2nd * (LengthDifference)); 
		//Emissions = Emissions + (Lambda * FrictionFactor * EngineSpeed * EngineDisplaceent * TimeDifference1);
		//Emissions = Emssions + (Lambda * Epsilon * Rho * LengthDifference *(LengthDifference/TimeDifference1)*(LengthDifference/TimeDifference1));
		//Emissions = Emissions - (Rho * Sigma2nd * Lambda * (LengthDifference) * Demand[RemovedCustomer[CounterRemoval]];
	}
...
void TS() {
	...
	while (IterO < OuterIter) {
		...
		while (Iter <= InnerIter) {
			...
			if (NoRoutes <= BestNoRoutes && Cost > CurrentCost&& Infeasibility == 0) {
				ProbAcceptance = exp((CurrentCost - Cost) / Temprature);
			...
			else if (NoRoutes <= BestNoRoutes && Cost < CurrentCost && Infeasibility == 0) {
				if (ValueS == 1) {
					Value1 += Beta;
				}
				if (ValueR == 1) {
	...
	fprintf(Out, "%f%s", ALNSEndTime - ALNSStartTime, "	");
