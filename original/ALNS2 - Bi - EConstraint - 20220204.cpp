...
** CurrentY,*** InitialV, ** InitialQ, ** InitialU, ** InitialY, FirstObj[10], 
SecondObj[10], EVLength,CVLength, ParetoEVLength[10], ParetoCVLength[10];
...
void ALNS() {
	...
	while (IterO < OuterIter) {
		...
		while (Iter <= InnerIter) {
			...
			if (NoRoutes <= BestNoRoutes && Emissions < EConstraint && Cost > CurrentCost && Infeasibility == 0) {
			...
			else if (NoRoutes <= BestNoRoutes && Emissions < EConstraint && Cost < BestCost && Infeasibility == 0) {
				if (ValueS == 1) {
					Value1 += Beta;
				}
				if (ValueR == 1) {
			...
			else if (NoRoutes <= BestNoRoutes && Emissions < EConstraint && Cost < CurrentCost && Infeasibility == 0) {
				if (ValueS == 1) {
					Value1 += Beta;
				}
				if (ValueR == 1) {
