...
int  NoVertices, NoStations, NoCustomers, NoVehicles, NoChargers, No = 0;   /////NoCopyStations
int *SortI, *SortII, *SortIII, *Seed, **Route, **BestRoute, **CurrentRoute, RoutedCustomers, 
	*Counter, *BestCounter, *CurrentCounter, *CounterStation, *BestCounterStation, *CurrentCounterStation, CounterRemoval, *Technology, SelectionNode1, SelectionNode2, Feasibility, FirstStation, *RemovedCustomer, *RemovalStation,
	*RemovalRoute, NoRoutes, BestNoRoutes, CurrentNoRoutes, InitialNoRoutes, NoRemovalC, *Score, *NoUse, NCR, NSR, NRR, NCI, NSI, NRI, NoOperators, IterC, IterS, EnergyLimit, TimeWindowCheck, Iter, IterO;
float *Xcoordinate, *Ycoordinate, *fCost, *BCost, *RCost,*TCost, *CurrentTCost, *BestTCost, *eTime, *lTime, *Demand, *MaxCapacity, *MaxEnergyCapacity, *CurrentMaxEnergyCapacity, *BestMaxEnergyCapacity, *ServiceTime, *Rate,
StartTime, EndTime, TSStartTime, TSEndTime, *EnergyConsumption,  *BestEnergyConsumption,  *CurrentEnergyConsumption, *AcquisitionCost, *CurrentAcquisitionCost, *BestAcquisitionCost, MaxDistance, tempLength1, tempLength2, *tempLength;
float *RouteLength, *BestRouteLength, *CurrentRouteLength, *Penalty, Test, Cost, BestCost, CurrentCost, **Weight, **Probability, *Pr, RouletteWheelPar, BigNumber;
float	***xPtr, ***vPtr, **qPtr, **yPtr, **wPtr, **uPtr, **AlfaPtr, **BetaPtr, **GammaPtr,  ***TempV, **TempQ, *TempU, **TempY, ***BestV, **BestQ, **BestU, **BestY, ***CurrentV, **CurrentQ, **CurrentU, **CurrentY;
float **t, **d, ***c;
float Epsilon, Ratio; //0.5
int NoEV, NoCV, CurrentNoEV, CurrentNoCV, BestNoEV, BestNoCV;
int *TabuStationRemoval, *TabuStationInsertion, *TabuCustomerRemoval, *TabuCustomerInsertion, TabuSign, Tenure;
...
void DistanceFunction () {	
	...
	//		d[i][j] = 1.5 * earth * cHarv; // Real distances
	//		d[j][i] = d[i][j];
	//		t[i][j] = 1 * d[i][j]; // With average speed of 60 km/h
	//		t[j][i] = t[i][j];
...
void TechnologySelectionI(int Variable1, int Variable2, int Variable3){
		...
		if( TempY[Variable1][Variable2] > 0){
			TempV[Variable1][Technology[Variable1]][Variable2] = MaxEnergyCapacity[Variable2] - TempY[Variable1][Variable2];
		}
		else{
			TempV[Variable1][Technology[Variable1]][Variable2] = MaxEnergyCapacity[Variable2];
		}
...
void TechnologySelectionII(int Variable1, int Variable2, int Variable3){
		if( TempY[Variable1][Variable3] < EnergyLimit ){
			if ( TempY[Variable1][Variable3] > 0 ){
				TempV[Variable1][Technology[Variable1]][Variable3] = MaxEnergyCapacity[Variable3] - TempY[Variable1][Variable3];
			}
			else{
				TempV[Variable1][Technology[Variable1]][Variable3] = MaxEnergyCapacity[Variable3];
			}
		}
...
void ReadData (int index1, int index2, int index3) {
	...
	NCI = 3;
	NSI = 3;
	NRI = 0;  // Do We Need This?
	NoOperators = NCR + NCI + NSR + NSI + NRR + NRI;
	IterC = 500; //??
	IterS = 1000; //??
	EnergyLimit = 1000;
	BigNumber = 999999;
	Cost = CurrentCost = BestCost = 0;
	NoEV = 0;
	NoCV = 0;
	Tenure = 2;
	...
	vPtr = new float** [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
	...
	qPtr = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices  + NoVertices ; i++) {
		qPtr[i] = new float[NoVehicles + 1];
	}

	yPtr = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		yPtr[i] = new float[NoVehicles + 1];
	}
	uPtr = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
	...
	tempLength = new float[NoVertices + 1];
	CurrentRouteLength = new float[NoVehicles + 1];
	Route = new int* [NoVertices   + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		Route[i] = new int[NoVehicles + 1];
	}
	CurrentRoute = new int* [NoVertices   + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		CurrentRoute[i] = new int[NoVehicles + 1];
	}
	BestRoute = new int* [NoVertices   + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
	...
	TabuStationRemoval = new int [NoVertices+1];
	TabuStationInsertion = new int [NoVertices+1];
	TabuCustomerRemoval = new int [NoVertices+1];
	TabuCustomerInsertion = new int [NoVertices+1];

	TempV = new float** [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices; i++) {
	...
	TempQ = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		TempQ[i] = new float[NoVehicles + 1];
	}
	TempY = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		TempY[i] = new float[NoVehicles + 2];
	}
	TempU = new float[NoVertices  + NoVertices + 1];

	CurrentV = new float** [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
	...
	CurrentQ = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		CurrentQ[i] = new float[NoVehicles + 1];
	}

	CurrentY = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		CurrentY[i] = new float[NoVehicles + 1];
	}
	CurrentU = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		CurrentU[i] = new float[NoVehicles + 1];
	}

	BestV = new float** [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
	...
	BestQ = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		BestQ[i] = new float[NoVehicles + 1];
	}

	BestY = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices; i++) {
		BestY[i] = new float[NoVehicles + 1];
	}
	BestU = new float* [NoVertices  + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices; i++) {
		BestU[i] = new float[NoVehicles + 1];
	}

	t = new float* [NoVertices + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices; i++) {
		t[i] = new float[NoVertices + 1];
	}

	d = new float* [NoVertices + NoVertices + 1];
	for (i = 0; i <= NoVertices + NoVertices; i++) {
	...
	for (k = 0; k <= NoVertices; k++){
		TabuStationRemoval[k] = 0;
	}
	for (k = 0; k <= NoVertices; k++){
		TabuStationInsertion[k] = 0;
	}
	for (k = 0; k <= NoVertices; k++){
		TabuCustomerRemoval[k] = 0;
	}
	for (k = 0; k <= NoVertices; k++){
		TabuCustomerInsertion[k] = 0;
	}
	for (i = 0; i <= NoVertices + NoVertices ; i++) {
		for (k = 0; k <= NoVehicles; k++) {
			Route[i][k] = 0;
...
void RemoveCustomer (int Variable1, int Variable2){
	...
	if(eTime[Route[Variable2][Variable1]] <= uPtr[Variable2][Variable1]){
		TimeDifference1 = t[Route[Variable2-1][Variable1]][Route[Variable2+1][Variable1]] - t[Route[Variable2-1][Variable1]][Route[Variable2][Variable1]] - t[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]] - ServiceTime[Route[Variable2][Variable1]];
	}
	else{
		TimeDifference1 = t[Route[Variable2-1][Variable1]][Route[Variable2+1][Variable1]] - t[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]] - ServiceTime[Route[Variable2][Variable1]]-eTime[Route[Variable2][Variable1]]; ///???
	}
	...
	Cost = Cost + (TCost[Variable1] * (LengthDifference));
	if(RouteLength[Variable1] != 0){
				...
				if(Technology[Route[i-1][Variable1]]> 0 && Technology[Route[i-1][Variable1]] <= 3){    //  Update Energy for the station
					if(yPtr[i-1][Variable1] < EnergyLimit){
						if( yPtr[i-1][Variable1] > 0 ) {
							vPtr[i-1][Technology[Route[i-1][Variable1]]][Variable1] = MaxEnergyCapacity[Variable1] - yPtr[i-1][Variable1];
							...
							vPtr[i-1][Technology[Route[i-1][Variable1]]][Variable1] = MaxEnergyCapacity[Variable1];
		...
		Counter[Variable1]--;
		if( MaxEnergyCapacity[Variable1] < EnergyLimit){
			Cost = Cost - RCost[1] * yPtr[Counter[Variable1]][Variable1];
		}
		...
		for( k = 1; k <= NoVehicles; k++){
			for(i = 1; i <= Counter[k]; i++){
				if(RemovalCustomer[j] == Route[i][k]){
					SelectedCustomer = i;
					SelectedRoute = k;
					TabuCustomerRemoval[Route[i][k]] = Tenure;
...
void RemoveStation (int Variable1, int Variable2){
	...
	RouteLength[Variable1] = RouteLength[Variable1] + d[Route[Variable2-1][Variable1]][Route[Variable2+1][Variable1]] - d[Route[Variable2-1][Variable1]][Route[Variable2][Variable1]] - d[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]];
	...
	Cost = Cost + (TCost[Variable1] * (d[Route[Variable2-1][Variable1]][Route[Variable2+1][Variable1]] - d[Route[Variable2-1][Variable1]][Route[Variable2][Variable1]] - d[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]]));
	...
	if(RouteLength[Variable1] > 0){
		...
		for( int i = Variable2; i <= Counter[Variable1]-1; i++){
			if( Route[i][Variable1] > NoCustomers){
				if(Technology[Route[i][Variable1]] > 0 && Technology[Route[i][Variable1]] <= 3){
					Cost = Cost + RCost[Technology[Route[i][Variable1]]] * vPtr[i][Technology[Route[i][Variable1]]][Variable1];
				}
			}
		}
	}
			...
			for(i = 1; i <= Counter[k]; i++){
				if(Route[i][k] == RemovedCustomer[j]){
					if(Route[i-1][k] > NoCustomers && Route[i-1][k] < NoVertices){
						SelectedRoute = k;
						SelectedStation = i - 1;
						TabuStationRemoval[Route[i-1][k]] = Tenure;
...
void RandomRouteRemoval1(){
	...
	for( j = 1; j <= NoRemovalR; j++){
		...
		Cost = Cost - RCost[1] * Ratio * MaxEnergyCapacity[RemovalRoute[j]];
		Cost = Cost + RCost[1] * yPtr[Counter[RemovalRoute[j]]][RemovalRoute[j]];
		...
		for(i = 1; i <= Counter[RemovalRoute[j]]; i++){
			Cost = Cost - TCost[RemovalRoute[j]] * d[Route[i-1][RemovalRoute[j]]][Route[i][RemovalRoute[j]]];
			...
			if (MaxEnergyCapacity[RemovalRoute[j]] < EnergyLimit){
				if(Route[i][RemovalRoute[j]] > NoCustomers && Route[i][RemovalRoute[j]] < NoVertices){
					if(Technology[Route[i][RemovalRoute[j]]] > 0 && Technology[Route[i][RemovalRoute[j]]] <= 3){
						Cost = Cost - RCost[Technology[Route[i][RemovalRoute[j]]]] * vPtr[i][Technology[Route[i][RemovalRoute[j]]]][RemovalRoute[j]];
...
void GreedyRouteRemoval2(){
	...
	for(j = 1; j <= NoRemovalR; j++){
		...
		if (MaxEnergyCapacity[SelectedRoute] < EnergyLimit){
			Cost = Cost - RCost[1] * Ratio * MaxEnergyCapacity[SelectedRoute];
			Cost = Cost + RCost[1] * yPtr[Counter[SelectedRoute]][SelectedRoute];
		...
		for(i = 1; i <= Counter[SelectedRoute]; i++){
			Cost = Cost - TCost[SelectedRoute] * d[Route[i-1][SelectedRoute]][Route[i][SelectedRoute]];
			...
			if (MaxEnergyCapacity[SelectedRoute] < EnergyLimit){
				if(Route[i][SelectedRoute] > NoCustomers && Route[i][SelectedRoute] < NoVertices){
					if(Technology[Route[i][SelectedRoute]] > 0 && Technology[Route[i][SelectedRoute]] <= 3){
						Cost = Cost - RCost[Technology[Route[i][SelectedRoute]]] * vPtr[i][Technology[Route[i][SelectedRoute]]][SelectedRoute];
					}
				}
			}
		}
			...
			for (i = 1; i <= Counter[k]; i++) {
				for (j = 1; j <= NoRemovalS; j++) {
					if (Route[i][k] == RemovalStation[j]) {
						SelectedStation = i;
						SelectedRoute = k;
						TabuStationRemoval[Route[i][k]] = Tenure;
						RemoveStation(SelectedRoute, SelectedStation);
...
void CustomerInsertion(int Variable1, int Variable2, int Variable3){
	...
	RouteLength[Variable1] = RouteLength[Variable1] + d[Route[Variable2][Variable1]][RemovedCustomer[Variable3]] + d[RemovedCustomer[Variable3]][Route[Variable2+1][Variable1]] - d[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]];
		...
		if(Technology[Route[CounterSpecial][Variable1]] > 0 && Technology[Route[CounterSpecial][Variable1]] <= 3){    //  Update Energy for the next station
			...
			if (yPtr[CounterSpecial+1][Variable1] < EnergyLimit ){
				if ( yPtr[CounterSpecial+1][Variable1] > 0 ){
					vPtr[CounterSpecial+1][Technology[Route[CounterSpecial][Variable1]]][Variable1] = MaxEnergyCapacity[Variable1] - yPtr[CounterSpecial+1][Variable1];
				}
				else{
					vPtr[CounterSpecial+1][Technology[Route[CounterSpecial][Variable1]]][Variable1] = MaxEnergyCapacity[Variable1];
				}
			}
		}
	}
	...
	Cost = Cost + (TCost[Variable1] * (d[Route[Variable2][Variable1]][Route[Variable2+1][Variable1]] + d[Route[Variable2+1][Variable1]][Route[Variable2+2][Variable1]] - d[Route[Variable2][Variable1]][Route[Variable2+2][Variable1]]));
	...
	TabuCustomerInsertion[Route[Variable2+1][Variable1]] = Tenure;
	Counter[Variable1]++;
	RoutedCustomers++;
	for (i = Variable2; i <= Counter[Variable1]; i++) {
		for (m = 1; m <= NoChargers; m++){
		...
		if(Technology[Route[CounterSpecial][Variable3]] > 0 && Technology[Route[CounterSpecial][Variable3]] <= 3){    //  Update Energy for the next station
			...
			if (TempY[CounterSpecial+1][Variable3] < EnergyLimit ){
				if (TempY[CounterSpecial+1][Variable3] > 0 ){
					TempV[CounterSpecial+1][Technology[Route[CounterSpecial][Variable3]]][Variable3] = MaxEnergyCapacity[Variable3] - TempY[CounterSpecial+1][Variable3];
				}
				else{
					TempV[CounterSpecial+1][Technology[Route[CounterSpecial][Variable3]]][Variable3] = MaxEnergyCapacity[Variable3];
				}
			}
		}
	}
...
void StationInsertion(int Variable1, int Variable2){
	...
	if( Route[Variable1-1][Variable2] > NoCustomers){
		TempY[Variable1][Variable2] = MaxEnergyCapacity[Variable2] - EnergyConsumption[Variable2] * d[Route[Variable1-1][Variable2]][SortII[NoCustomers+1]];
	}
	else{
	...
	if( TempY[Variable1][Variable2] < EnergyLimit){
		if( TempY[Variable1][Variable2] > 0 ){
			if(Technology[SortII[NoCustomers+1]] > 0 && Technology[SortII[NoCustomers+1]] <= 3){
				TempV[Variable1][Technology[SortII[NoCustomers+1]]][Variable2] = MaxEnergyCapacity[Variable2] - TempY[Variable1][Variable2];
			}
		}
		else{
			TempV[Variable1][Technology[SortII[NoCustomers+1]]][Variable2] = MaxEnergyCapacity[Variable2];
		}
	}
	...
	if(CounterSpecial > 0){
		...
		for(j = Variable1; j <= CounterSpecial; j++){
			if(Technology[SortII[NoCustomers+1]] > 0 && Technology[SortII[NoCustomers+1]] <= 3){
				...
				TempY[j+1][Variable2] = yPtr[j][Variable2] + MaxEnergyCapacity[Variable2] - EnergyConsumption[Variable2] * d[Route[Variable1-1][Variable2]][SortII[NoCustomers+1]] - EnergyConsumption[Variable2] * d[SortII[NoCustomers+1]][Route[Variable1][Variable2]] + EnergyConsumption[Variable2] * d[Route[Variable1-1][Variable2]][Route[Variable1][Variable2]];
			}							 
		}
		...
		if( TempY[CounterSpecial+1][Variable2] < EnergyLimit){
			if( TempY[CounterSpecial+1][Variable2] > 0 ){
				TempV[CounterSpecial+1][Technology[Route[CounterSpecial][Variable2]]][Variable2] = MaxEnergyCapacity[Variable2] - TempY[CounterSpecial+1][Variable2];
			}
			else{
				TempV[CounterSpecial+1][Technology[Route[CounterSpecial][Variable2]]][Variable2] = MaxEnergyCapacity[Variable2];
			}
		}
	...
	if(TimeWindowCheck == 0){
		...
		RouteLength[Variable2] = RouteLength[Variable2] + d[Route[Variable1-1][Variable2]][SortII[NoCustomers+1]] + d[SortII[NoCustomers+1]][Route[Variable1][Variable2]] - d[Route[Variable1-1][Variable2]][Route[Variable1][Variable2]];
		for (i = Variable1; i <= Counter[Variable2]; i++) {
			for (m = 1; m <= NoChargers; m++){
				Cost += vPtr[i][m][Variable2]*RCost[m];
			}
		}
		Cost = Cost + (TCost[Variable2] * (d[Route[Variable1-1][Variable2]][SortII[NoCustomers+1]] + d[SortII[NoCustomers+1]][Route[Variable1][Variable2]] - d[Route[Variable1-1][Variable2]][Route[Variable1][Variable2]]));
		...
		TabuStationInsertion[Route[Variable1][Variable2]] = Tenure;
		CounterStation[SortII[NoCustomers+1]]++;
		Counter[Variable2]++;
		if (MaxEnergyCapacity[Variable2] < EnergyLimit ){
			Cost -= RCost[1] * yPtr[Counter[Variable2]][Variable2];
			...
			for(k = 1; k <= NoVehicles; k++){ // InitialNoRoutes
				if( Counter[k] == 0 ){
					...
					for(i = 0; i <= Counter[k]-1; i++){
						...
						Distance = Distance + MaxDistance*NoiseParameter*random_number;
						if(Distance < Min && TempQ[Counter[k]][k] >=  0){ /// Check for load capacity
						// Check for time window
						CheckTimeWindowFunction(i, j, k);
						if(TimeWindowCheck == 0){
...
void InitialSolutionI(int index1, int index2, int index3, int index4){
	...
	PenaltySwap = SwapTime;
	for( i = NoCustomers + 1; i <= NoCustomers + NoStations; i++){
		if(PenaltySwap < Penalty[SortIII[1]]){ 
			Technology[i] = 4;
		}
		else{
			Technology[i] = SortIII[1];
		}
	}*/
								...
								if( TempU[SortI[m]]<= lTime[SortI[m]] ){ //Check Time window for the selected node  ??
									...
									for( i = Counter[k]+2 ; i <= Counter[k]+3; i++){
										if( Route[i][k] > NoCustomers && Route[i][k] < NoVertices){
											...
											vPtr[i-1][Technology[Route[i-1][k]]][k] = MaxEnergyCapacity[k] - yPtr[i-1][k];
...
void TS(){
	float TempCost, r, PercentRemovalC, Temprature, Sum;
	int i, j, m, k, s, OuterIter, InnerIter, RouteRemovalIter, RouteIter, Sigma1, Sigma2, Sigma3, Beta, OperatorCounter, Infeasibility, Random;
	int NoLevel2, NoLevel3;
	...
	while ( IterO < OuterIter ){
		...
		while( Iter <= InnerIter ){
			...
			if( NoRoutes <= BestNoRoutes && Cost < BestCost && Infeasibility == 0){
				if( ValueS == 1){
					for( k = 1; k <= NoRoutes; k ++){
						for( i = 0; i <= Counter[k]; i++){
							if( TabuStationInsertion[Route[i][k]] > 0){
								TabuStationInsertion[Route[i][k]] = 0;
							}
							if( TabuStationRemoval[Route[i][k]] > 0){
								TabuStationRemoval[Route[i][k]] = 0;
							}
						}
					}
					Value1 += Beta;
				...
				if(ValueR == 1){
					for( k = 1; k <= NoRoutes; k ++){
						for( i = 0; i <= Counter[k]; i++){
							if( TabuCustomerInsertion[Route[i][k]] > 0){
								TabuCustomerInsertion[Route[i][k]] = 0;
							}
							if( TabuCustomerRemoval[Route[i][k]] > 0){
								TabuCustomerRemoval[Route[i][k]] = 0;
							}
						}
					}
					Value2 += Beta;
			...
			if( NoRoutes <= BestNoRoutes && Infeasibility == 0){
				if( ValueS == 1){
					TabuSign = 0;
					for( k = 1; k <= NoRoutes; k ++){
						for( i = 0; i <= Counter[k]; i++){
							if( TabuStationInsertion[Route[i][k]] > 0){
								TabuStationInsertion[Route[i][k]]--;
								TabuSign = 1;
				...
				if(ValueR == 1){
					...
					for( k = 1; k <= NoRoutes; k ++){
						for( i = 0; i <= Counter[k]; i++){
							if( TabuCustomerInsertion[Route[i][k]] > 0){
								TabuCustomerInsertion[Route[i][k]]--;
								TabuSign = 1;
			...
			else if( NoRoutes <= BestNoRoutes && Cost < BestCost && Infeasibility == 0){
				if( ValueS == 1){
						Value1 += Beta;
					}
					if(ValueR == 1){
	...
	for (k = 1; k <= BestNoRoutes; k++) {
		if (BestY[BestCounter[k]][k] > 0) {
			for (i = BestCounter[k] - 1; i >= 1; i--) {
				for (m = 1; m <= NoChargers; m++) {
					if (BestV[i][m][k] > BestY[BestCounter[k]][k]) {
						BestV[i][m][k] -= BestY[BestCounter[k]][k];
						for (j = i + 1; j <= BestCounter[k]; j++) {
							BestY[j][k] -= BestY[BestCounter[k]][k];
						}
						i = 0;
						break;
					}
					else if (BestV[i][m][k] > 0 && BestV[i][m][k] < BestY[BestCounter[k]][k]) {
						BestV[i][m][k] = 0;
						for (j = i + 1; j <= BestCounter[k]; j++) {
							BestY[j][k] -= BestY[BestCounter[k]][k];
							BestRoute[j - 1][k] = BestRoute[j][k];
	...
	for (k = 1; k <= BestNoRoutes; k++) {
		fprintf(Out, "%s%d%s", "Route ", k, "	");
		fprintf(Out, "%s", "Loads	");
		fprintf(Out, "%s", "Distances	");
		fprintf(Out, "%s", "Energy Levels	");
		fprintf(Out, "%s", "Times	");
		fprintf(Out, "%s", "\n");
		for (i = 1; i <= BestCounter[k]; i++) {
			fprintf(Out, "%d%s", BestRoute[i][k], "	");
			fprintf(Out, "%f%s", BestQ[i][k], "	");
			fprintf(Out, "%f%s", d[BestRoute[i-1][k]][BestRoute[i][k]], "	");
			fprintf(Out, "%f%s", BestY[i][k], "	");
			fprintf(Out, "%f%s", BestU[i][k], "\n");
		}
	}
	fprintf(Out, "%s", "\n");
	for (k = 1; k <= BestNoRoutes; k++) {
		for (i = 0; i <= BestCounter[k]; i++) {
			for (m = 1; m <= NoChargers; m++) {
				fprintf(Out, "%s%d%s%d%s%d%s%f%s", "Final: v[", i, "][", m, "][", k, "] = ", BestV[i][m][k], "\n");
			}
			fprintf(Out, "%s", "\n");
		}
		fprintf(Out, "%s", "\n");
	...
	/*for(k = 1; k <= BestNoRoutes; k++){
		for(i = 0; i <= BestCounter[k]-1; i++){
			BestCost -= (TCost[k] * d[BestRoute[i][k]][BestRoute[i+1][k]]);
		}
	}*/
	fprintf(Out, "%s%f%s", "Final: TotalCost= ", BestCost, "\n");
	BestCost = 0;
	/*for(k = 1; k <= BestNoRoutes; k++){
		for(i = 0; i <= BestCounter[k]-1; i++){
			BestCost += (d[BestRoute[i][k]][BestRoute[i+1][k]]);
		}
	}*/
	float ChargingCost = 0;
	float ACost = 0;
	float TravelingCost = 0;
	for (k = 1; k <= BestNoRoutes; k++) {
		for (i = 0; i <= BestCounter[k] - 1; i++) {
			BestCost += (BestTCost[k] * d[BestRoute[i][k]][BestRoute[i + 1][k]]);
			TravelingCost += (BestTCost[k] * d[BestRoute[i][k]][BestRoute[i + 1][k]]);
	...
	for (k = 1; k <= BestNoRoutes; k++) {
		if (BestRoute[1][k] != 0) {
			if (BestMaxEnergyCapacity[k] < EnergyLimit) {
				BestCost += (RCost[1] * Ratio * BestMaxEnergyCapacity[k]);
				ChargingCost += (RCost[1] * Ratio * BestMaxEnergyCapacity[k]);
				BestCost += BestAcquisitionCost[k];
				ACost += BestAcquisitionCost[k];
			}
			else {
				BestCost += BestAcquisitionCost[k];
				ACost += BestAcquisitionCost[k];
	...
	for (k = 1; k <= BestNoRoutes; k++) {
		if (BestMaxEnergyCapacity[k] < EnergyLimit) {
			BestCost -= (RCost[1] * BestY[BestCounter[k]][k]);
			ChargingCost -= (RCost[1] * BestY[BestCounter[k]][k]);
		}
	}
	for (k = 1; k <= BestNoRoutes; k++) {
		for (i = 1; i <= BestCounter[k] - 1; i++) {
			for (m = 1; m <= NoChargers; m++) {
				BestCost += (RCost[m] * BestV[i][m][k]);
				if (BestV[i][m][k] > 0) {
					if (m == 2) {
						NoLevel2++;
					}
	...
	TSEndTime = second();
	fprintf(Out, "%s", "TotalCost 	");
	fprintf(Out, "%s", "AcquisitionCost 	");
	fprintf(Out, "%s", "ChargingCost 	");
	fprintf(Out, "%s", "TravelingCost 	");
	fprintf(Out, "%s", "Time 	");
	fprintf(Out, "%s", "EVLength 	");
	fprintf(Out, "%s", "CVLength 	");
	fprintf(Out, "%s", "NoEV 	");
	fprintf(Out, "%s", "NoCV 	");
	fprintf(Out, "%s", "NoLevel2 	");
	fprintf(Out, "%s", "NoLevel3 	");
	fprintf(Out, "%s", "\n");
	fprintf(Out, "%f%s", BestCost, "	");
	fprintf(Out, "%f%s", ACost, "	");
	fprintf(Out, "%f%s", ChargingCost, "	");
	fprintf(Out, "%f%s", TravelingCost, "	");
	fprintf(Out, "%f%s", TSEndTime - TSStartTime, "	");
	fprintf(Out, "%f%s", EVLength, "	");
	fprintf(Out, "%f%s", CVLength, "	");
	fprintf(Out, "%d%s", NoEV, "	");
	fprintf(Out, "%d%s", NoCV, "	");
	fprintf(Out, "%d%s", NoLevel2, "	");
	fprintf(Out, "%d%s", NoLevel3, "	");
	//////// Delete ///////
...
void main (void) {
	///// This is for full recharge /////
	int i, j, k, l;
	for (int i = 11; i <= 11; i++) { //1-2
		for (j = 1; j <= 1; j++) { //1
			for (k = 1; k <= 1; k++) {//1-3         
				for (l = 5; l <= 5; l++) {//1- 29 2-35 3 -28
