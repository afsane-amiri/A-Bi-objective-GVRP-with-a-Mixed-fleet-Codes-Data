...
float Emissions, CurrentEmissions, BestEmissions;
float **t, **d, ***c;
...
void ReadData (int index1, int index2, int index3) {
	...
	Emissions = CurrentEmissions = BestEmissions = 0;
	NoEV = 0;
	NoCV = 0;
	Tenure = 2;
...
void InitialSolutionI(int index1, int index2, int index3, int index4){
	...
	for(k = 1; k <= NoRoutes; k++){
		for(i = 0; i <= Counter[k]-1; i++){
			...
			if (MaxEnergyCapacity[k] > EnergyLimit) {
				Emissions += (Lambda * CurbWeight * 60 * Rho * Sigma2nd * (d[Route[i][k]][Route[i + 1][k]])); 
				Emissions += (Lambda * EngineFriction * EngineSpeed * EngineDisplacement * d[Route[i][k]][Route[i + 1][k]] * InverseSpeed);
				Emissions += (Lambda * Epsilon2nd * Rho * 60 * d[Route[i][k]][Route[i + 1][k]] * Speed * Speed);
				Emissions += (Rho * Sigma2nd * Lambda * 60 *(d[Route[i][k]][Route[i + 1][k]]) * qPtr[i][k]);
			}
		}
	}
	for(k = 1; k <= NoRoutes; k++){
	...
	for (k = 1; k <= BestNoRoutes; k++) {
		for (i = 0; i <= BestCounter[k] - 1; i++) {
			if (BestMaxEnergyCapacity[k] > EnergyLimit) {
				Emissions += (Lambda * CurbWeight * 60 * Rho * Sigma2nd * (d[BestRoute[i][k]][BestRoute[i + 1][k]])); 
				Emissions += (Lambda * EngineFriction * EngineSpeed * EngineDisplacement * d[BestRoute[i][k]][BestRoute[i + 1][k]] * InverseSpeed);
				Emissions += (Lambda * Epsilon2nd * Rho * 60 * d[BestRoute[i][k]][BestRoute[i + 1][k]] * Speed * Speed);
				Emissions += (Rho * Sigma2nd * Lambda * 60 *(d[BestRoute[i][k]][BestRoute[i + 1][k]]) * BestQ[i][k]);
			}
		}
	}
