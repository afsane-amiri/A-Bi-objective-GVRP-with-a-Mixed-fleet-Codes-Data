...
void RemoveCustomer(int Variable1, int Variable2) {
	...
	if (RouteLength[Variable1] != 0) {
		...
		if (MaxEnergyCapacity[Variable1] < EnergyLimit) {
			...
			Cost -= AcquisitionCost[Variable1];
			//Emissions -= (Rho * Sigma2nd * Lambda * ( qPtr[Variable2 - 1][Variable1] * d[Route[Variable2 - 1][Variable1]][Route[Variable2 ][Variable1]] + qPtr[Variable2][Variable1] * d[Route[Variable2][Variable1]][Route[Variable2 + 1][Variable1]]));
			NoCV--;
		}
...
void CustomerInsertion(int Variable1, int Variable2, int Variable3) {
	...
	if (Counter[Variable1] == 1) {
		...
		//if (MaxEnergyCapacity[Variable1] > EnergyLimit) {
		//	Emissions += (Rho * Sigma2nd * Lambda * ( qPtr[Counter[Variable1]][Variable1] * d[Route[0][Variable1]][Route[Counter[Variable1]][Variable1]]));
		//}
	}
	//else {
	//	if (MaxEnergyCapacity[Variable1] > EnergyLimit) {
	...
	for (i = Counter[Variable1]; i > Variable2; i--) {
		...
		//if (MaxEnergyCapacity[Variable1] > EnergyLimit) {
		//	for (j = Variable2 + 1; j <= Counter[Variable1]-1; j++) {
		//		Emissions -= Rho * Sigma2nd * Lambda * d[Route[j][Variable1]][Route[j+1][Variable1]] * Demand[RemovedCustomer[Variable3]];
		//	}
		//}
	}
...
void InitialSolutionI(int index1, int index2, int index3, int index4) {
	...
	*/
	...
	fprintf(Out, "%s%f%s", "TotalEmissions= ", Emissions, "\n");
}
...
void ALNS() {
	...
	OuterIter = 999;
	InnerIter = 250; //2500
	RouteIter = 10; //1250 Maybe make it less is better idea
	ProbAcceptance = 0;
	IterO = 0;
	Sigma1 = 15;
	Sigma2 = 10; //10
	...
	while (IterO < OuterIter) {
		...
		while (Iter <= InnerIter) {
			...
			WeightedSum = Weight2nd * Cost + (1 - Weight2nd) * Emissions;
			if (NoRoutes <= BestNoRoutes && WeightedSum > CurrentWeightedSum && Infeasibility == 0) {
			...
			else if (NoRoutes <= BestNoRoutes && WeightedSum < BestWeightedSum && Infeasibility == 0) {
				if (ValueS == 1) {
					Value1 += Beta;
				}
				if (ValueR == 1) {
			...
			else if (NoRoutes <= BestNoRoutes && WeightedSum < CurrentWeightedSum && Infeasibility == 0) {
				if (ValueS == 1) {
					Value1 += Beta;
				}
				if (ValueR == 1) {
